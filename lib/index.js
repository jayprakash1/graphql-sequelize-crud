  ...
  var connArgWhr = function connArgWhr(model, args, options) {
    ...
    _.forOwn(args, function (value, key) {
      if (model.customConnectionArgs && model.customConnectionArgs[key]) {
        var cond = model.customConnectionArgs[key].whrClause(value, options);
          ...
          connectionFields: {
            total: {
              ...
              resolve: function resolve(source, args, context, info) {
                ...
                return source.source[accessors.count]({ requestUser: context ? context.user : null, where: connArgWhr(target, source.args, source) });
              }
            }
          },
          before: function before(findOptions, args, context, info) {
